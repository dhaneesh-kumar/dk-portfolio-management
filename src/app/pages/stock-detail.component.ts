  ...
  template: `
    @if (stock() && portfolio()) {
        ...
        <header class="bg-white shadow-sm border-b border-slate-200">
          <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
            <div class="flex items-center gap-4">
              ...
              <div>
                <h1 class="text-3xl font-bold text-slate-900">{{stock()!.ticker}}</h1>
          ...
          <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-2 space-y-6">
              ...
              <div class="bg-white rounded-xl shadow-lg border border-slate-200 p-6">
                <h2 class="text-xl font-bold text-slate-900 mb-4">Market Data</h2>
                
                @if (stock()!.marketData) {
                  <div class="grid grid-cols-2 md:grid-cols-4 gap-6">
                    <div class="text-center">
                      <div class="text-3xl font-bold text-slate-900">₹{{stock()!.marketData!.price | number:'1.2-2'}}</div>
                      <div class="text-sm text-slate-500">Current Price</div>
                      <div class="mt-1" [class]="stock()!.marketData!.changePercent >= 0 ? 'text-green-600' : 'text-red-600'">
                        {{stock()!.marketData!.changePercent >= 0 ? '+' : ''}}{{stock()!.marketData!.changePercent}}%
                      </div>
                    </div>
                    
                    <div class="text-center">
                      <div class="text-2xl font-bold text-slate-900">{{stock()!.marketData!.pe | number:'1.1-1'}}</div>
                      <div class="text-sm text-slate-500">P/E Ratio</div>
                    </div>
                    
                    <div class="text-center">
                      <div class="text-2xl font-bold text-slate-900">₹{{stock()!.marketData!.eps | number:'1.1-1'}}</div>
                      <div class="text-sm text-slate-500">EPS</div>
                    </div>
                    
                    <div class="text-center">
                      <div class="text-2xl font-bold text-slate-900">{{stock()!.marketData!.dividendYield | number:'1.2-2'}}%</div>
                      <div class="text-sm text-slate-500">Dividend Yield</div>
                    </div>
                  </div>
                  
                  <div class="mt-6 grid grid-cols-2 md:grid-cols-4 gap-6 pt-6 border-t border-slate-200">
                    <div>
                      <div class="text-sm text-slate-500">Book Value</div>
                      <div class="text-lg font-semibold text-slate-900">₹{{stock()!.marketData!.bookValue | number:'1.2-2'}}</div>
                    </div>
                    
                    <div>
                      <div class="text-sm text-slate-500">Market Cap</div>
                      <div class="text-lg font-semibold text-slate-900">₹{{(stock()!.marketData!.marketCap / 10000000) | number:'1.0-0'}}Cr</div>
                    </div>
                    
                    <div>
                      <div class="text-sm text-slate-500">Debt Ratio</div>
                      <div class="text-lg font-semibold text-slate-900">{{stock()!.marketData!.debt | number:'1.1-1'}}</div>
                    </div>
                    
                    <div>
                      <div class="text-sm text-slate-500">Volume</div>
                      <div class="text-lg font-semibold text-slate-900">{{(stock()!.marketData!.volume / 100000) | number:'1.0-0'}}L</div>
                    </div>
                  </div>
                } @else {
                  <div class="text-center py-8">
                    ...
                    <h3 class="text-lg font-medium text-slate-900 mb-2">No Market Data Available</h3>
                    <p class="text-slate-600">Market data will be fetched from API integration</p>
                  </div>
                }
              </div>
              ...
              <div class="bg-white rounded-xl shadow-lg border border-slate-200 p-6">
                ...
                @if (stock()!.notes.length > 0) {
                    ...
                    @for (note of stock()!.notes; track note.id) {
                      <div class="border border-slate-200 rounded-lg p-4">
                        <div class="flex items-center justify-between mb-2">
                          <h3 class="font-semibold text-slate-900">{{note.section}}</h3>
                  ...
                  <div class="text-center py-8">
                    ...
                    <p class="text-slate-600 mb-4">Add your thoughts, analysis, and strategy for this stock</p>
                    <button 
                      (click)="showAddNoteModal.set(true)"
            ...
            <div class="space-y-6">
              <!-- Position Summary -->
              <div class="bg-white rounded-xl shadow-lg border border-slate-200 p-6">
                <h2 class="text-xl font-bold text-slate-900 mb-4">Your Position</h2>
                ...
                <div class="space-y-4">
                  ...
                  <div class="flex justify-between">
                    ...
                    <span class="font-semibold">₹{{stock()!.currentPrice | number:'1.2-2'}}</span>
                  </div>
                  
                  <div class="flex justify-between">
                    <span class="text-slate-600">Total Value</span>
                    <span class="font-semibold">₹{{stock()!.totalValue | number:'1.0-0'}}</span>
                  </div>
              ...
              <!-- Quick Note Sections -->
              <div class="bg-white rounded-xl shadow-lg border border-slate-200 p-6">
                <h3 class="font-semibold text-slate-900 mb-4">Quick Add Notes</h3>
                <div class="space-y-2">
              ...
              <form (ngSubmit)="saveNote()" #form="ngForm">
                ...
                <div class="mb-6">
                  ...
                  <textarea 
                    ...
                    rows="8"
                    class="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                    placeholder="Write your analysis, thoughts, or strategy..."></textarea>
                </div>
      ...
      <div class="min-h-screen flex items-center justify-center">
        <div class="text-center">
          <h1 class="text-2xl font-bold text-slate-900 mb-4">Stock Not Found</h1>
