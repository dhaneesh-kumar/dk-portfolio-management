...
import { Portfolio, Stock, MarketData, StockNote } from '../models/portfolio.model';
...
export class PortfolioService {
  ...
  addStockToPortfolio(portfolioId: string, stock: Omit<Stock, 'id' | 'notes' | 'totalValue'>): void {
  ...
  updateStockWeight(portfolioId: string, stockId: string, weight: number): void {
    const portfolio = this.getPortfolio(portfolioId);
    if (!portfolio) return;
  ...
  addStockNote(portfolioId: string, stockId: string, section: string, content: string): void {
    const portfolio = this.getPortfolio(portfolioId);
    if (!portfolio) return;
  ...
  updateStockNote(portfolioId: string, stockId: string, noteId: string, content: string): void {
    const portfolio = this.getPortfolio(portfolioId);
    if (!portfolio) return;
