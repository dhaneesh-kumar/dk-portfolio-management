    ...
    <div class="bg-white rounded-xl shadow-lg border border-slate-200 p-6">
      ...
      @if (type === 'pie') {
        <div class="flex items-center justify-center">
          <svg #svgElement width="300" height="300" class="drop-shadow-sm">
          </svg>
        </div>
          ...
          @for (segment of chartData; track segment.label) {
            <div class="flex items-center gap-3">
              ...
              <div class="flex-1 min-w-0">
                <div class="text-sm font-medium text-slate-900 truncate">{{segment.label}}</div>
                <div class="text-xs text-slate-500">{{segment.percentage}}% (₹{{segment.value | number:'1.0-0'}})</div>
              </div>
            </div>
          }
        </div>
      ...
      @if (type === 'bar') {
        <div class="space-y-3">
          @for (item of chartData; track item.label; let i = $index) {
            <div class="flex items-center gap-3">
              <div class="w-20 text-sm font-medium text-slate-700 truncate">{{item.label}}</div>
              <div class="flex-1">
                <div class="flex items-center gap-2">
                  ...
                  <div class="text-sm font-medium text-slate-900 w-12 text-right">{{item.percentage}}%</div>
                </div>
              </div>
            </div>
          }
...
export class PortfolioChartComponent implements OnInit, AfterViewInit {
  ...
  private colors = [
    ...
    '#06B6D4', '#F43F5E', '#8B5A2B', '#6B7280', '#DC2626'
  ];
  ...
  private prepareChartData() {
    ...
    const totalValue = this.stocks.reduce((sum, stock) => sum + stock.totalValue, 0);
    
    this.chartData = this.stocks.map((stock, index) => ({
      label: stock.ticker,
  ...
  private drawPieChart() {
    ...
    this.chartData.forEach((segment, index) => {
      ...
      path.setAttribute('class', 'transition-all duration-300 hover:opacity-80');
