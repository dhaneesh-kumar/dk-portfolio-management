...
import { fromEventPattern, Subscription, ReplaySubject, map, Observable } from 'rxjs';
...
export class AuthService implements OnDestroy {
  ...
  async signUpWithEmail(email: string, password: string, displayName?: string): Promise<boolean> {
    return this.executeAuthAction(async () => {
      ...
      if (result.user) {
        ...
        this.notificationService.success("Account created!", "Welcome to Portfolio Management");
        return true;
      }
  ...
  async signOut(): Promise<void> {
    try {
      ...
      this.notificationService.error("Sign out failed", "An error occurred while signing out");
    }
  }
  ...
  private initializeFirebaseAuth(): void {
    try {
      ...
      this.handleAuthInitializationFailure("Firebase authentication not available");
    }
  }
  ...
  private setupAuthListener(): void {
    if (!this.auth) {
      ...
      this.handleAuthInitializationFailure("Authentication service unavailable");
      return;
    }
    ...
    this.authSubscription = authState$.subscribe({
      ...
      error: (error) => {
        ...
        this.handleAuthInitializationFailure("Authentication initialization failed");
      }
    });
  }
   ...
   */
  private handleAuthStateChange(firebaseUser: FirebaseUser | null, isFirstState: boolean): void {
    if (firebaseUser) {
      const user: User = {
   ...
   */
  private async executeAuthAction(action: () => Promise<boolean>): Promise<boolean> {
    try {
      ...
      this.logger.error("Authentication action failed", error, { code: error.code });
      this._error.set(errorMessage);
